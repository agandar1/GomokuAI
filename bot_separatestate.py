...
class Bot:
    def __init__(self, size):
        self.size = size
        self.state = GameState(19) 
        self.tree = MonteCarlo(self.state)
    ...
    def find_best_monos(self, board, e_board):
        ...
        bot_scores = [[m, self.state.mono_value(board, m, True)] for m in self.state.monomials]
        enemy_scores = [[m, self.state.mono_value(e_board, m, False)] for m in self.state.monomials]
    ...
    def start(self):
        ...
        self.state.board[move[0]][move[1]] = 2
    ...
    def turn(self, opponent_move):
        ...
        self.state.board[opponent_move[0]][opponent_move[1]] = 0
        self.tree.apply_move(opponent_move)
        self.tree.set_turn(2)

        # calculate bot's and opponent's scores
        self.state.score_board = self.state.network(self.state.board)
        enemy_board = self.state.gen_opponent_board(self.state.board)
        enemy_scores = self.state.network(enemy_board)

        # see who is ahead
        best_monos = self.find_best_monos(self.state.board, enemy_board)
        if (best_monos[1][1] > best_monos[0][1] and best_monos[1][1] != 4):
            # block if enemy is ahead
            best_move = self.block(self.state.board, self.state.score_board, enemy_scores, best_monos)
        else:
            # ignore enemy and build if bot is ahead
            best_move = self.build(self.state.board, self.state.score_board, enemy_scores, best_monos[0]) 

        # place piece and send choice back to gui
        self.tree.search_tree()
        best_move = self.tree.choose_best_move()
        self.state.board[best_move[0]][best_move[1]] = 2
